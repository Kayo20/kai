...
// Dev helper: copy .env.example to .env (development only)
app.post('/internal/setup-env', (req, res) => {
  ...
  try {
    ...
    if (fs.existsSync(dest)) {
      return res.status(409).json({ error: '.env already exists' });
    }
    fs.copyFileSync(src, dest);
    console.log('Copied .env.example to .env');
    return res.json({ ok: true, message: 'Copied .env.example to .env' });
  } catch (err) {
